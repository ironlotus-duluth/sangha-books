...
2. **Vote** by picking their top 3 choices using ranked voting (Borda count — 1st choice
   = 3 pts, 2nd = 2 pts, 3rd = 1 pt; highest total wins)
3. **See results** live, with a visual chart showing point breakdowns
...
| `vote.html` | Click-to-select top-3 ranked voting interface |
| `results.html` | Live results with point breakdown visualization (Borda count) |
...
| `app.js` | All shared JavaScript — API client, data cache, Borda count algorithm, session management, utilities |
...
- **Borda count runs client-side.** The voting calculation happens in the browser (`app.js` → `RCV.calculate()` — named `RCV` for historical reasons but uses Borda count), not on the server. The server just stores raw vote rankings.
